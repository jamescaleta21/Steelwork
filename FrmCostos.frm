...
'pub_cadena = "SELECT FAR_FECHA, FAR_COSPRO_SUP, FAR_CANTIDAD, FAR_SIGNO_ARM, FAR_STOCK, FAR_COSPRO FROM FACART WHERE FAR_CODCIA = ? AND FAR_FECHA < ? AND FAR_CODART = ? and far_estado <>'E'  and far_transito <> 'P'  ORDER BY FAR_CODCIA, FAR_FECHA, FAR_SIGNO_ARM DESC , FAR_TIPMOV, FAR_NUMOPER2"
pub_cadena = "SELECT FAR_FECHA, FAR_COSPRO_SUP, FAR_CANTIDAD, FAR_SIGNO_ARM, FAR_STOCK, FAR_COSPRO,FAR_PRECIO FROM FACART WHERE FAR_CODCIA = ? AND FAR_FECHA < ? AND FAR_CODART = ? and far_estado <>'E'  and far_transito <> 'P'  ORDER BY FAR_CODCIA, FAR_FECHA,  FAR_NUMOPER2"
...
'pub_cadena = "SELECT FAR_NUM_LOTE,FAR_PRECIO_NETO,FAR_COSPRO_ANT,FAR_COSPRO_SUP, FAR_STOCK , FAR_TIPO_CAMBIO, FAR_COSTEO_REAL, FAR_NUMGUIA, FAR_NUMSER, FAR_NUMFAC, FAR_OTRA_CIA, FAR_CODART, FAR_COSPRO, FAR_FLETE, FAR_BRUTO, FAR_DESCTO, FAR_GASTOS, FAR_TOT_DESCTO, FAR_SIGNO_ARM, FAR_CANTIDAD, FAR_TIPMOV, FAR_MONEDA, FAR_FECHA_COMPRA, FAR_COSTEO, FAR_ESTADO, FAR_PRECIO, FAR_EQUIV,FAR_JABAS FROM FACART WHERE FAR_CODCIA = ? AND FAR_CODART = ? AND FAR_FECHA >= ? AND  FAR_FECHA <= ?   and far_estado <>'E' and far_transito <> 'P'   ORDER BY FAR_CODCIA, FAR_FECHA, FAR_SIGNO_ARM DESC ,FAR_TIPMOV, FAR_NUMOPER2"
pub_cadena = "SELECT FAR_NUM_LOTE,FAR_PRECIO_NETO,FAR_COSPRO_ANT,FAR_COSPRO_SUP, FAR_STOCK , FAR_TIPO_CAMBIO, FAR_COSTEO_REAL, FAR_NUMGUIA, FAR_NUMSER, FAR_NUMFAC, FAR_OTRA_CIA, FAR_CODART, FAR_COSPRO, FAR_FLETE, FAR_BRUTO, FAR_DESCTO, FAR_GASTOS, FAR_TOT_DESCTO, FAR_SIGNO_ARM, FAR_CANTIDAD, FAR_TIPMOV, FAR_MONEDA, FAR_FECHA_COMPRA, FAR_COSTEO, FAR_ESTADO, FAR_PRECIO, FAR_EQUIV,FAR_JABAS FROM FACART WHERE FAR_CODCIA = ? AND FAR_CODART = ? AND FAR_FECHA >= ? AND  FAR_FECHA <= ?   and far_estado <>'E' and far_transito <> 'P'   ORDER BY FAR_CODCIA, FAR_FECHA,  FAR_NUMOPER2"
...
pub_cadena = "SELECT Distinct FAR_CODART FROM FACART WHERE FAR_CODART <> 0 and FAR_CODCIA= ? AND FAR_FECHA >= ?  AND FAR_FECHA <= ?  AND  FAR_ESTADO <> 'E'   ORDER BY FAR_CODART"
If CHEART.Value = 1 Then
  pub_cadena = "SELECT Distinct FAR_CODART FROM FACART WHERE FAR_CODCIA= ? AND FAR_FECHA >= ?  AND FAR_FECHA <= ? AND FAR_CODART= " & WCODART & "  AND  FAR_ESTADO <> 'E'   ORDER BY FAR_CODART"
End If
If Val(TLINEA.Text) <> 0 Then
  WLINEA = Val(TLINEA.Text)
  ' PRUEBA
  pub_cadena = "SELECT Distinct FAR_CODART FROM FACART, ARTI WHERE (ART_CODCIA = FAR_CODCIA) AND (FAR_CODART = ART_KEY) AND ART_LINEA = " & WLINEA & " AND FAR_CODCIA= ? AND FAR_FECHA >= ?  AND FAR_FECHA <= ? AND FAR_ESTADO<>'E'  ORDER BY FAR_CODART"
End If
' TEMPORAL
...
'    End If
 ...
 '  Far_Cost.Requery
   ...
   If Far_Cost!FAR_costeo = "A" And Far_Cost!far_estado <> "E" And Far_Cost!far_signo_arm = 1 And LK_EMP <> "PIU" And Far_Cost!FAR_TIPMOV = 20 Then
     ' WCOSPRO_SUP = Nulo_Valor0(Far_Cost!FAR_COSPRO_SUP)
     'WCOSPRO_SUP = Nulo_Valor0(Far_Cost!FAR_PRECIO)
      WCOSPRO_SUP = Nulo_Valor0(Far_Cost!FAR_PRECIO) '/ ((100 + LK_IGV) / 100))
     ' ws_cospro = Nulo_Valor0(Far_Cost!FAR_COSPRO_SUP)
      'ws_cospro = Nulo_Valor0(Far_Cost!FAR_PRECIO)
      ws_cospro = Nulo_Valor0(Far_Cost!FAR_PRECIO) '/ ((100 + LK_IGV) / 100))
   Else
      WCOSPRO_SUP = Nulo_Valor0(Far_Cost!FAR_COSPRO)
      WCOSPRO_SUP = arm_llave!ARM_COSPRO
      ws_cospro = Nulo_Valor0(Far_Cost!FAR_COSPRO)
      ws_cospro = arm_llave!ARM_COSPRO
   End If
   ...
   Do Until Far_Cost.EOF
      ...
      If WS_SALDO < 0 Then WS_SALDO = 0  'agregado gts 15/07/2010
      
      If Far_Cost!FAR_MONEDA = "D" Then WS_TIPO_CAMBIO = JALAR(Far_Cost!FAR_fecha_compra, Far_Cost!FAR_fecha_compra)
      Far_Cost.Edit
      If Far_Cost!FAR_costeo = "A" And Far_Cost!far_estado <> "E" And Far_Cost!far_signo_arm = 1 And Far_Cost!FAR_TIPMOV = 20 Then
      ' es temporal para regulizar costos
        ...
        ' ws_valor_ant = (WS_SALDO - Far_Cost!far_cantidad * wk_equiv) * ws_cospro
         ...
         ws_igv = 0
         If Far_Cost!far_JABAS = 1 Then
         ws_compra = WS_TIPO_CAMBIO * Far_Cost!far_cantidad * ((Far_Cost!FAR_PRECIO / ((100 + LK_IGV) / 100)) / wk_equiv)  '- ((Far_Cost!FAR_DESCTO) / ((100 + LK_IGV) / 100) * WS_TIPO_CAMBIO) + ((Far_Cost!FAR_FLETE) / ((100 + LK_IGV) / 100) * WS_TIPO_CAMBIO)
         Else
         ws_compra = WS_TIPO_CAMBIO * Far_Cost!far_cantidad * ((Far_Cost!FAR_PRECIO) / wk_equiv) ' - ((Far_Cost!FAR_DESCTO) * WS_TIPO_CAMBIO) + ((Far_Cost!FAR_FLETE) * WS_TIPO_CAMBIO)
         End If
         ws_ajuste = 0
         If LK_EMP = "3AA" Then
         ...
         Else
           ...
           End If
         End If
         'WS_AJUSTE2 = WS_AJUSTE2 - (Nulo_Valor0(Far_Cost!FAR_DESCTO) * WS_TIPO_CAMBIO)  'quitado gts
         ws_valor4 = Far_Cost!far_cantidad
         ws_compra = (ws_compra + ws_ajuste) + WS_AJUSTE2
         'ws_valor5 = ws_compra
         Far_Cost!far_precio_neto = ws_compra
         'If Far_Cost!far_JABAS = 1 Then
         ultimo_costo = (Far_Cost!FAR_PRECIO) '/ ((100 + LK_IGV) / 100))
         'Else
        ...
        ' End If
         ...
         If WS_SALDO <> 0 Then
            If ws_valor_ant < 0 Then
            ws_cospro = arm_llave!ARM_COSPRO
            Else
            ws_cospro = (ws_valor_ant + ws_compra) / WS_SALDO 'Val(Format(((Far_Cost!FAR_PRECIO / Far_Cost!FAR_equiv) - (Far_Cost!FAR_DESCTO / Far_Cost!far_cantidad) + (Far_Cost!FAR_FLETE / Far_Cost!far_cantidad)) * WS_TIPO_CAMBIO, "0.0000"))
            End If
         Else
            ws_cospro = (ws_valor_ant + ws_compra) / Far_Cost!far_cantidad 'Val(Format(((Far_Cost!FAR_PRECIO / Far_Cost!FAR_equiv) - (Far_Cost!FAR_DESCTO / Far_Cost!far_cantidad) + (Far_Cost!FAR_FLETE / Far_Cost!far_cantidad)) * WS_TIPO_CAMBIO, "0.0000"))
         End If
         'ws_valor7 = ws_cospro
...
'            Else
'            ws_cospro = Val(Format(((Far_Cost!FAR_PRECIO / Far_Cost!FAR_equiv) - (Far_Cost!FAR_DESCTO / Far_Cost!far_cantidad) + (Far_Cost!FAR_FLETE / Far_Cost!far_cantidad)) * WS_TIPO_CAMBIO, "0.0000")) / 1.19
'            End If
'         End If
      End If
NADA:
...
Loop
        ...
        If pre_mayor.EOF Then MsgBox "Error en Precios..."
            Do Until pre_mayor.EOF
              ...
              'pre_mayor!pre_pordes1 = ((pre_mayor!pre_pre1) / (Nulo_Valor0(pre_mayor!PRE_cosTO_ant) * 119))
              
              If ws_valor4 > 0 Then
                  pre_mayor!PRE_COSTO = Format(ws_compra * pre_mayor!PRE_EQUIV / ws_valor4, "0.00")   'ws_cospro * pre_mayor!PRE_EQUIV '
                  pre_mayor!PRE_COSTO = ws_cospro * pre_mayor!PRE_EQUIV
              Else
                  pre_mayor!PRE_COSTO = Nulo_Valor0(pre_mayor!PRE_cosTO_ant) 'ws_cospro * pre_mayor!PRE_EQUIV '
                  pre_mayor!PRE_COSTO = ws_cospro * pre_mayor!PRE_EQUIV
                  
              End If
              'agregado gts para actualizar precio de venta en base al % de utilidad
             ' pre_mayor!PRE_PRE1 = Round(pre_mayor!PRE_COSTO * ((100 + LK_IGV) / 100) * ((100 + pre_mayor!PRE_POR1) / 100), 1)
...
'txtfecha.Text = "01/" & Format(DatePart("m", LK_FECHA_DIA), "00") & "/" & DatePart("yyyy", LK_FECHA_DIA)  QUITADO GTS
