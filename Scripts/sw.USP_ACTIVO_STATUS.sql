...
(
    ...
    WHERE s.ROUTINE_TYPE = 'PROCEDURE' -- Validación del tipo
          ...
          AND s.ROUTINE_NAME = 'USP_ACTIVO_STATUS' -- Validación del nombre
)
BEGIN
    DROP PROC [sw].[USP_ACTIVO_STATUS];
END;
GO
/*
SW.USP_ACTIVO_STATUS '01',1,0,'WOFLS'
*/
CREATE PROCEDURE [sw].[USP_ACTIVO_STATUS]
(
    @codCia CHAR(2),
    @activoId INT,
    @activo BIT,
...
BEGIN
    ...
    (
       ...
       AND (@activo = 1)
    BEGIN
        SET @message = 'Activo ya se encuentra Habilitado.';
        SET @code = -1;
        GOTO Terminar;
    END;
    ...
    (
       ...
       AND (@activo = 0)
    BEGIN
        SET @message = 'Activo ya se encuentra Desabilitado.';
        SET @code = -2;
        GOTO Terminar;
    END;
